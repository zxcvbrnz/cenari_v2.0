        ...
        <div class="overflow-y-auto flex-1">
            @forelse($contacts as $contact)
                <button 
                    ...
                    <p class="text-xs text-gray-500">{{ \Carbon\Carbon::parse($contact->last_chat)->diffForHumans() }}</p>
                </button>
            @empty
    ...
    <div class="w-2/3 flex flex-col bg-gray-50">
        @if($selectedPhone)
            ...
            <div class="p-4 border-b bg-white font-bold text-gray-700 flex justify-between items-center">
                ...
                <span class="text-xs font-normal text-amber-600 bg-amber-50 px-2 py-1 rounded border border-amber-200">
            ...
            <div class="flex-1 p-4 overflow-y-auto flex flex-col space-y-3 bg-slate-100">
                ...
                @foreach($messages as $msg)
                    <div class="flex flex-col {{ $msg->direction === 'outbound' ? 'items-end' : 'items-start' }}">
                        <div class="max-w-[70%] rounded-lg p-3 text-sm {{ $msg->direction === 'outbound' ? 'bg-emerald-600 text-white rounded-br-none' : 'bg-white text-gray-800 border rounded-bl-none shadow-sm' }}">
                            <p>{{ $msg->body }}</p>
